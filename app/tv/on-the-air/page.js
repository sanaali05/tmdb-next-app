...
const page = () => {
    ...
    useEffect(()=>{
        const fetch= async()=>{
            const {data}= await axios.get(`/tv/on_the_air?api_key=b6330e59d17035fa955abd01c083646b&page=${currentPage}`)
            const onTheAirData = data.results
    ...
    return (
        <>
            <section className='on-the-air'>
                <h1>on tv /on the air  </h1>
